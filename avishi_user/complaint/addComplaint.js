...
function checkInputs()
{
    ...
    {
        setErrorFor(description, "Description should contain at least 5 characters");
        flag = 1;
    }
    else if(description.length > 100)
    {
        setErrorFor(description, "Description should contain at most 100 characters");
        flag = 1;
    }
    else
    {
...
httpReq.open("POST", "http://localhost:8080/ntsf_backend_war/complaint", true);
httpReq.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
httpReq.send("action=createComplaint" + "&user_id=" + user_id + "&title=" + title + "&description=" + description);


function addComplaintData(httpReq)
{
    let jsonAddComplaintResponse = JSON.parse(httpReq.responseText); /* Here when we recieve the response from the server, we convert it to JSON format as it 
    ...
    */
    ...
    if(jsonAddComplaintResponse.status === "success")
    {
        ...
        window.location.href = "http://localhost:8080/ntsf_frontend_war/user/complaints.html";
    }
    else
    {
